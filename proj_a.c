...
void inserir(Arvore **raiz, int i, int j, int k, char animal[], char *matriz_car[][10]){
    if (*raiz == NULL){
        *raiz = malloc (sizeof (Arvore)); 
        ...
        //SEMELHANCA TA SEMPRE DANDO 4 AJEITAR UU
        if (semelhanca <= 2){
...
int main(){
    //MUDAR O CODIGO PRA RECEBER A QUANTIDADE DE>>
    //=>CARACTERISTICAS, ESPECIES, E (MUTACOES)---QNDO O CODIGO TIVER FUNCIONANDO CERTINHO
    Arvore *raiz = NULL;
    int op, i, j, k;
    i = 0;
    k = 0;
    ...
    while(op != 0 && i < 10){
        ...
        switch(op){
            ...
            case 1:
                ...
                k++;
                printf("\ndigite seu animal\n");
                //Ta dando erro aqui, com no segundo round ja ta comendo um espaco de caracter da string DESCOBRIR!!!! UU
                scanf("%s" , &matriz_ani[i]);
            ...
            case 2:
                //printar na parte de baixo as caracteristicas que mais aparecem no maior tipo de especies U
                imprimir(raiz);
                break;
                //DESENHAR a arvore quando terminar U
